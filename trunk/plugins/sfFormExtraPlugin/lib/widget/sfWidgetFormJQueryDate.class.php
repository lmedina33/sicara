  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
    {
      ...
      return $this->getOption('date_widget')->render($name,substr($value, 0, 10), $attributes, $errors).
             $this->renderTag('input', array('type' => 'hidden', 'size' => 10, 'id' => $id = $this->generateId($name).'_jquery_control', 'disabled' => 'disabled')).
             sprintf(<<<EOF
             <script type="text/javascript">
                function wfd_%s_read_linked()
                {
                  ...
                  try{
                  %s_beforeShow(jQuery("#%s").datepicker('getDate'));
                  }catch(err){}
                  return {};
                }
                ...
                function wfd_%s_update_linked(date)
                {
                  jQuery("#%s").val(date.substring(0, 10));
                  try{
                  %s_onSelect(jQuery("#%s").datepicker('getDate'));
                  }catch(err){}
                }
...
EOF
      ,
        ...
        $prefix,$prefix,
        $id,$id,
        $id,$prefix,$prefix,$this->getOption('disabled'),$image, $this->getOption('culture'), $this->getOption('config')
      );
    } 
}
