...
function WriteHTML($h, $html){
    ...
    while(count(explode('<',$html))>2){
        ...
        switch($aux[0]){
            case "ol":{
                $aux2=explode('</ol>',$aux[1],2);
                $html=  str_replace('</li>', '', $aux2[0]);
                $aux3=explode('<li>',$html);
                $html="";
                for($i=1;$i<count($aux3);$i++){
                    $html.=$i.". ".$aux3[($i)];
                }
                $html.=$aux2[1];
            }break;
            case "ul":{
                $aux2=explode('</ul>',$aux[1],2);
                $html=  str_replace('<li>', '- ', $aux2[0]);
                $html=  str_replace('</li>', '', $html);
                $html.=$aux2[1];
            }break;
            case "strong":{
                $aux2=explode('</strong>',$aux[1],2);
                $this->SetFont($this->FontFamily, 'B');
                $this->Write($h,$this->clearHTML($aux2[0]));
                $html=$aux2[1];
                $this->SetFont($this->FontFamily);
            }break;
            case "em":{
                $aux2=explode('</em>',$aux[1],2);
                $this->SetFont($this->FontFamily, 'I');
                $this->Write($h,$this->clearHTML($aux2[0]));
                $html=$aux2[1];
                $this->SetFont($this->FontFamily);
            }break;
            case 'span style="text-decoration: underline;"':{
                $aux2=explode('</span>',$aux[1],2);
...
function clearHTML($html){
    ...
    $html=  str_replace('span style="text-decoration: underline;"', "", $html);
    $html=  str_replace('</span>', "", $html);
    
    $html=  str_replace('span style="text-decoration: line-through;"', "", $html);
    $html=  str_replace('</span>', "", $html);
