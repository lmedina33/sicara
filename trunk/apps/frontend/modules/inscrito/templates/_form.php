...
<form action="<?php echo url_for('inscrito/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?numero_formulario='.$form->getObject()->getNumeroFormulario() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('Delete', 'inscrito/delete?id_inscrito='.$form->getObject()->getNumeroFormulario(), array('method' => 'delete', 'confirm' => 'Are you sure?')) ?>
          <?php endif; ?>
