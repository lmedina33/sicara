...
<h1>Ver Inscrito a Curso Empresarial</h1>
<a href="<?php echo url_for('curCurso/verInscritos?id='.$curso->getIdCurCurso()) ?>" class="button back">Volver</a>
<a href="<?php echo url_for('curFormulario/editFull?id='.$form->getObject()->getIdCurFormulario()) ?>" class="button edit">Activar Edición</a>
<form id="form" action="<?php echo url_for('curFormulario/' . ($form->getObject()->isNew() ? 'createFull' : 'updateFull') . (!$form->getObject()->isNew() ? '?id_cur_formulario=' . $form->getObject()->getIdCurFormulario() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <div class="curDatos">
        ...
        <?php } ?>
            <br />
            <label>Fecha de Inscripción:</label> <?php echo $form->getObject()->getCreatedAt() ?>
        <br />
    </div>
